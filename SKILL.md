...
2. **需求与设计** — define observable acceptance criteria and the smallest coherent approach. For user-operable behavior, define a user-journey contract from a realistic starting state through a discoverable entry, meaningful next actions, and a usable end state; reject orphan capabilities that exist in code but cannot be reached or completed by the intended user. Fast work may use a few inline bullets. Standard/High-risk work should make scope, non-goals, affected contracts, failure behavior, verification, and consequential alternatives traceable. Obtain a design decision when a missing choice materially changes the solution.
...
4. **适当验证** — select evidence from the acceptance criteria and risk, not from a fixed ritual. Run targeted unit tests and relevant lint/type/build checks; add broader, integration, API, container, or real-browser verification when the changed behavior requires it. User-facing behavior needs a real browser and explicit visible oracle. Begin from the journey's realistic entry rather than a hidden deep link or preconstructed final state, then verify the changed step can be reached, the user can identify what to do next, and the resulting state supports the promised downstream action. Test setup may establish unrelated preconditions but must not bypass the behavior being accepted. Backend-only work may mark browser testing `N/A` with equivalent integration/API evidence from the real consumer entry. Record exact commands, outcomes, skipped checks, and baseline failures. A required check that cannot run makes verification `partial` or `blocked`, never complete.
...
- Do not accept a user-operable capability without a reachable entry, satisfiable preconditions, an understandable next action, visible outcome, and a usable or recoverable end state. Direct-route testing alone does not prove discoverability or journey closure.
...
- user-journey start, entry, critical steps, closure, and any intentionally bypassed preconditions;
...
For detailed track selection and gate checks, read [references/stage-checklists.md](references/stage-checklists.md) when the work is Standard, High-risk, blocked, cross-cutting, or resumed after context loss. For any change to a user-operable flow, read [references/user-journey-validation.md](references/user-journey-validation.md) before finalizing design or verification.
